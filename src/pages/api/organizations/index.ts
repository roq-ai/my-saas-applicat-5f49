...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.carbon_data?.length > 0) {
      const create_carbon_data = body.carbon_data;
      body.carbon_data = {
        create: create_carbon_data,
      };
    } else {
      delete body.carbon_data;
    }
    if (body?.carbon_goal?.length > 0) {
      const create_carbon_goal = body.carbon_goal;
      body.carbon_goal = {
        create: create_carbon_goal,
      };
    } else {
      delete body.carbon_goal;
    }
